 ...
 */
package io.gravitee.alerter.standalone.bootstrap;
...
public class Bootstrap {
    ...
    private final static String CONTAINER_CLASS = "io.gravitee.alerter.standalone.container.Container";
    ...
    private void checkInstallRoot(File graviteeHomeDir) {
        ...
        if (!graviteeHomeDir.isDirectory()) {
            throw new RuntimeException("Invalid Gravitee Alerter Standalone Home. Not a directory: "
                    + graviteeHomeDir.getAbsolutePath());
        }
        ...
        File [] files = graviteeLibDir.listFiles(pathname -> {
            return pathname.getName().startsWith("gravitee-alerter-standalone-bootstrap");
        });

        if (files == null || files.length == 0 || files.length > 1) {
            throw new RuntimeException("Invalid Gravitee Alerter Standalone Home. No bootstrapable jar can be " +
                    "found in " + graviteeLibDir.getAbsolutePath());
        }
    }

    /**
     * Start the Gravitee Alerter Standalone daemon.
     */
